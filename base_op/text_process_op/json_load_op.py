...
class JsonLoadOp(Op):
    ...
    Href:
        https://git.corp.kuaishou.com/ad-aigc-algo-engine/video-graph/-/blob/master/video_graph/ops/base_op/text_process_op/json_load_op.py?ref_type=heads

    Examples:
        from video_graph.ops.base_op.text_process_op.json_load_op import *
        ...
        # 执行算子
        success = join_load_op.process(op_context)
    ...
    def compute(self, op_context: OpContext) -> bool:
        ...
        for index, row in in_table.iterrows():
            json_str = row.get(source_column)
            in_table.at[index, target_column] = json.loads(json_str)

        op_context.output_tables.append(in_table)
        return True


op_register.register_op(JsonLoadOp) \
