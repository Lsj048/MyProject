...
from video_graph.common.utils.parse_script import encode_script, parse_script, merge_text, split_subtitle
from video_graph.data_table import DataTable
from video_graph.op import Op, op_register
from video_graph.op_context import OpContext


class ExpandScriptV2Op(Op):
    """
    Function:
        对当前脚本进行扩展
    ...
    Href:
       https://git.corp.kuaishou.com/ad-aigc-algo-engine/video-graph/-/blob/master/video_graph/ops/extend_op/script_process_op/expand_script_v2_op.py?ref_type=heads
    ...
    def compute(self, op_context: OpContext) -> bool:
        ...
        for index, row in material_table.iterrows():
            ...
            for _text in llm_input:
                video_asr.append(_text)
            if video_asr:
                asr_list.append(",".join(video_asr))
        ...
        while _retry_num < 5:
            try:
                ...
                subtitles = photo_asr_client.sync_req(
                    ...
                    rag_data=json.dumps({"appendix": pre_subtitles}),
            ...
            if subtitles:
                ...
                modify_script = merge_text(modify_script)
                modify_script = split_subtitle(modify_script)
        ...
        #     self.fail_reason = f"脚本未生成直播镜号"
        #     self.trace_log.update({"fail_reason": self.fail_reason})
        #     op_context.output_tables.append(material_table)
        #     return False
        # if not check_dsp_live_montage(subtitles):
        #     self.fail_reason = f"脚本台词出现<号召进入直播间>等话术"
        #     self.trace_log.update({"fail_reason": self.fail_reason})
        #     op_context.output_tables.append(material_table)
        #     return False
...
op_register.register_op(ExpandScriptV2Op) \
